...
# Test F1
if ((wanted_tests_length == 0 or "F1" in wanted_tests) and ("F1" not in unwanted_tests)):

    test_F1_corpus = ["XYZ", "KAR", "INI", "IRI",
                      "SIN", "ASI", "LIR", "ALI", "KIT", "AND", "RIP"]

    test_F1_calculations = the3.place_words(
        test_F1_corpus
    )

    test_F1_is_false = False

    test_F1 = (
        "F1",
        test_F1_calculations,
        test_F1_corpus,
        test_F1_is_false
    )
...
# Test F2
if ((wanted_tests_length == 0 or "F2" in wanted_tests) and ("F2" not in unwanted_tests)):

    test_F2_corpus = ["ODTU", "METU", "ERDA", "TRAP", "ROAM",
                      "ROOT", "REMY", "YOUT", "KLIK", "OREO", "TAUT"]

    test_F2_calculations = the3.place_words(
        test_F2_corpus
    )

    test_F2_is_false = False

    test_F2 = (
        "F2",
        test_F2_calculations,
        test_F2_corpus,
        test_F2_is_false
    )

    tests.append(test_F2)

# Test F3
if ((wanted_tests_length == 0 or "F3" in wanted_tests) and ("F3" not in unwanted_tests)):

    test_F3_corpus = ["OLE", "TEL", "WIL",
                      "LAW", "WHO", "ALI", "CAT", "COW", "ALO"]

    test_F3_calculations = the3.place_words(
        test_F3_corpus
    )

    test_F3_is_false = False

    test_F3 = (
        "F3",
        test_F3_calculations,
        test_F3_corpus,
        test_F3_is_false
    )

    tests.append(test_F3)

# Test F4
if ((wanted_tests_length == 0 or "F4" in wanted_tests) and ("F4" not in unwanted_tests)):

    test_F4_corpus = ["AB", "AC", "CB", "CD", "BD", "AA"]

    test_F4_calculations = the3.place_words(
        test_F4_corpus
    )

    test_F4_is_false = False

    test_F4 = (
        "F4",
        test_F4_calculations,
        test_F4_corpus,
        test_F4_is_false
    )

    tests.append(test_F4)
